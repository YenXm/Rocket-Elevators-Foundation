...
class DimCustomersControllerTest < ActionDispatch::IntegrationTest
  setup { @dim_customer = dim_customers(:one) }
  ...
  test 'should create dim_customer' do
    assert_difference('DimCustomer.count') { post dim_customers_url, params: { dim_customer: {} } }
  ...
  test 'should destroy dim_customer' do
    assert_difference('DimCustomer.count', -1) { delete dim_customer_url(@dim_customer) }
