...
class ColumnsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create column' do
    assert_difference('Column.count') do
      post columns_url,
           params: {
             column: {
               batterie_id: @column.batterie_id,
               information: @column.information,
               notes: @column.notes,
               number_of_floors_served: @column.number_of_floors_served,
  ...
  test 'should destroy column' do
    assert_difference('Column.count', -1) { delete column_url(@column) }
