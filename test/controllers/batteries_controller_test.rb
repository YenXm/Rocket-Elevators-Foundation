...
class BatteriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create battery' do
    assert_difference('Battery.count') do
      post batteries_url,
           params: {
             battery: {
               Date_of_: @battery.Date_of_,
               building_id: @battery.building_id,
               certificate_of_operations: @battery.certificate_of_operations,
               commissioning: @battery.commissioning,
  ...
  test 'should destroy battery' do
    assert_difference('Battery.count', -1) { delete battery_url(@battery) }
