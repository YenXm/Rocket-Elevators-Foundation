...
emails =
  Array[
    ...
    'mathieu.lefrancois@codeboxx.biz',
    'david.larochelle@codeboxx.biz',
    'nicolas.pineault@codeboxx.biz',
    'david.amyot@codeboxx.biz',
    'remi.gagnon@codeboxx.biz',
    'timothy.wever@codeboxx.biz',
    'kiril.kleinerman@codeboxx.biz',
    'emmanuela.derilus@codeboxx.biz',
    'abdul.akeeb@codeboxx.biz',
    'krista.sheely@codeboxx.biz',
    'jonathan.murray@codeboxx.biz'
  ]
...
first_names.each_with_index do |first_name, index|
  ...
  if index == 0
    ...
    @user =
      User.create(
        email: emails[index],
        password: Faker::Internet.password(min_length: 30, max_length: 40, mix_case: true, special_characters: true),
        first_name: first_names[index],
        last_name: last_names[index],
        function: functions[index]
      )
...
while $j < 250
  ...
  if random_building_type == 'Residential'
    ...
    if random_product_line == 'Standard'
      ...
      totalElevatorPrice = totalShafts * 12_345
      installationFees = totalElevatorPrice / 100 * 13
      finalPrice = totalElevatorPrice + installationFees
    else
      totalElevatorPrice = totalShafts * 15_400
...
while $i < $num
  Faker::UniqueGenerator.clear if $i == 146
...
while $i < 100
  ...
  while $j < random
    ...
    random = rand(0..3)
    @building_details =
      BuildingDetail.create!(building: @buildings, information_key: 'Type', value: values_type[random])
    $j += 1
