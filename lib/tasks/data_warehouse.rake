...
#         i = rand(1..4)
#         notes = Faker::Lorem.sentence(word_count: rand(5..15)) if i == 1
#         return notes
#     elsif status == 'Fail'
...
#         PsqlQuery.execute(
#             'CREATE TABLE public.fact_quotes (creation_date date NULL,
...
#         PsqlQuery.execute(
#             'CREATE TABLE public.fact_contacts (creation_date date NULL,
#             company_name varchar NULL,
#             email varchar NULL,
...
#         PsqlQuery.execute(
#             'CREATE TABLE public.fact_elevators (date_of_commissionig date NULL,
...
#         PsqlQuery.execute(
#             'CREATE TABLE public.dim_customers (creation_date date NULL,
#             company_name varchar NULL,
#             email varchar NULL,
...
#         PsqlQuery.execute(
#             'CREATE TABLE public.fact_interventions (
...
#                 query =
#                     "insert into fact_quotes(quote_id,creation_date , company_name, email, nb_elevator ) values('#{quote.id}', '#{quote.created_at}','#{quote.company_name.gsub("'", "''")}', ' #{quote.email}',  '#{quote.amount_elevators}')"
...
#                 query =
#                     "insert into fact_contacts(contact_id, creation_date, company_name, email, project_name) values('#{contact.id}', '#{contact.date_of_creation}', '#{contact.company_name}', '#{contact.email}', '#{contact.project_name}')"
#                 PsqlQuery.execute(query)
#                 lead_bar.increment
#             end
...
#                             query =
#                                 "insert into fact_elevators(serial_number, date_of_commissionig, building_city, customer_id, building_id) values('#{elevator.id}', '#{elevator.Date_of_commissioning}', '#{@address.city}', '#{building.customer.id}', '#{building.id}')"
#                             PsqlQuery.execute(query)
#                         end
#                     end
#                 end
...
#                 query =
#                     "insert into dim_customers(creation_date, company_name, full_name, email, nb_elevator, customer_city) values('#{customer.customer_creation_date}', '#{customer.company_name.gsub("'", "''")}', '#{customer.full_name_of_the_company_contact.gsub("'", "''")}', '#{customer.email_of_the_company_contact}', '#{get_total_ele(customer.id)}', '#{customer.address.city}')"
#                 PsqlQuery.execute(query)
